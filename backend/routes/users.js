...
// POST /api/users/verify - verify a torn API key (used by Discord bot)
router.post('/verify', async (req, res) => {
  try {
    ...
    if (!tornCheck.valid) {
      return res.status(400).json({ success: false, error: tornCheck.error });
    }
    ...
    res.json({
      success: true,
      torn_id: tornCheck.torn_id,
      torn_name: tornCheck.torn_name,
      level: tornCheck.level
    });
  } catch (err) {
...
// GET /api/users/by-discord/:discord_id — bot uses this to look up torn_id
...
// GET /api/users/:torn_id/claims - get seller's active claims
