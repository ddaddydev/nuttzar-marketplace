...
function createContract(data) {
  const db = getDb();
  const { type, buyer_torn_id, buyer_torn_name, target_torn_id, target_torn_name, seller_price_per_unit, quantity, bounty_amount = 0 } = data;
...
function activateContract(contractId) {
  getDb().prepare(`
    UPDATE contracts SET status='active', payment_confirmed=1, payment_confirmed_at=unixepoch(), updated_at=unixepoch()
    WHERE id = ?
  `).run(contractId);
...
function completeClaim(claimId) {
  return completeClaimPartial(claimId, null); // null = full completion
}

// Partial completion — credit only verifiedCount units, return the rest to pool
function completeClaimPartial(claimId, verifiedCount) {
  ...
  // Only queue payout if something was actually credited
  if (credited > 0) {
  ...
  // Return uncredited units to pool
  if (returned > 0) {
    db.prepare(`UPDATE contracts SET quantity_remaining=quantity_remaining+?, updated_at=unixepoch() WHERE id=?`)
...
module.exports = {
  ...
  getContract, getContractByUuid, claimUnits, completeClaim, expirestaleClaims,
  markPayoutSent, getClaim, getPendingPayouts, completeClaimPartial, CLAIM_LIMITS, MIN_PRICES, FEE,
};
